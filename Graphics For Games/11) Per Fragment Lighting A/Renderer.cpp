...
{
	camera = new Camera(0.0f,0.0f,Vector3(RAW_WIDTH*HEIGHTMAP_TEX_X / 2.0f, 500, RAW_HEIGHT*HEIGHTMAP_Z));
	//camera = new Camera();

	heightMap = new HeightMap(TEXTUREDIR"terrain.raw");
	currentShader = new Shader(SHADERDIR"PerPixelVertex.glsl",SHADERDIR"PerPixelFragment.glsl");
	//currentShader = new Shader(SHADERDIR"PerPixelVertex.glsl", SHADERDIR"TexturedFragment.glsl");
