...
Renderer::Renderer(Window &parent) : OGLRenderer(parent) {
	rotation = 0.0f;
	camera = new Camera(0.0f, 0.0f, Vector3(RAW_WIDTH*HEIGHTMAP_TEX_X / 2.0f, 500, RAW_WIDTH*HEIGHTMAP_TEX_X));
	...
	for (int x = 0; x < LIGHTNUM; ++x) {
		for (int z = 0; z < LIGHTNUM; ++z) {
			...
			l.SetPosition(Vector3(xPos, 1500.0f, zPos));
	...
	heightMap = new HeightMap("../../Textures/map3.raw");
...
void Renderer::FillBuffers() {
	...
	heightMap->Draw();
	modelMatrix = Matrix4::Translation(Vector3(3000, 2000, 3000)) * Matrix4::Scale(Vector3(100, 100, 100));
	UpdateShaderMatrices();
	sphere->Draw();
...
void Renderer::DrawPointLights() {
	...
	for (int x = 0; x < LIGHTNUM; ++x) {
		for (int z = 0; z < LIGHTNUM; ++z) {
			...
			modelMatrix = pushMatrix*Matrix4::Rotation(1, Vector3(0, 1, 0)) * popMatrix * Matrix4::Translation(l.GetPosition()) * Matrix4::Scale(Vector3(radius, radius, radius));
