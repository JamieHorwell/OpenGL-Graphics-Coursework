...
void Renderer::RenderScene() {
	//flush colour and depth buffer, depth buffer gets flushed to 1.0 - very furthest edge of NDC space
	...
	//bind texture unit 0 to diffuseTex in shaders 
	...
	for (unsigned int i = 0; i < 2; ++i) {
		...
		//3glBindTexture(GL_TEXTURE_2D, textures[i]);
		meshes[i]->Draw();
	}
	glUseProgram(0);
	SwapBuffers();
