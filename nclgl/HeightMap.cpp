...
HeightMap::HeightMap(std::string name)
{
	...
	//create our indices, grab vertice indexes that form the square patches, which is in turn formed from 2 triangles
...
HeightMap::HeightMap()
{
	...
	for (int x = 0; x < RAW_WIDTH; ++x) {
		for (int z = 0; z < RAW_HEIGHT; ++z) {
			...
			vertices[offset] = Vector3(x * HEIGHTMAP_X, 1 * HEIGHTMAP_Y, z * HEIGHTMAP_Z);
